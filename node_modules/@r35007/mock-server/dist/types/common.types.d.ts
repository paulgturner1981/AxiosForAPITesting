...
export type UserMiddlweares = {
    [x: string]: express.RequestHandler | Array<express.RequestHandler>;
};
export type GlobalMiddlweares = {
    globals?: express.RequestHandler | Array<express.RequestHandler>;
};
export type HarMiddleware = {
    harEntryCallback?: (entry: HarEntry, routePath: string, routeConfig: UserTypes.RouteConfig) => {
        [key: string]: UserTypes.RouteConfig;
    };
    harDbCallback?: (data: string | UserTypes.Db | {
        [key: string]: Omit<object, '__config'> | any[] | string;
    } | HAR, dbFromHAR: UserTypes.Db) => UserTypes.Db;
};
export type KibanaMiddleware = {
    kibanaHitsCallback?: (hit: HIT, routePath: string, routeConfig: UserTypes.RouteConfig) => {
        [key: string]: UserTypes.RouteConfig;
    };
    kibanaDbCallback?: (data: string | UserTypes.Db | {
        [key: string]: Omit<object, '__config'> | any[] | string;
    } | HAR, dbFromKibana: UserTypes.Db) => UserTypes.Db;
};
